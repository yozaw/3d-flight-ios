...
class Animate3DGraphicViewController: UIViewController, MissionSettingsVCDelegate, UIAdaptivePresentationControllerDelegate {
    ...
    //private var missionFileNames = ["GrandCanyon.csv", "Hawaii.csv", "Pyrenees.csv", "Snowdon.csv", "ManualOperation"]
    private var missionFileNames = ["グランドキャニオン", "マニュアル", "ハワイ", "ピレネー", "スノードン"]
    ...
    private var isAnimating = false {
        didSet {
            self.playBBI?.title = isAnimating ? "停止" : "再生"
        }
    }
    ...
    @objc func operate() {
        ...
        let startPoint = AGSPoint(x: 138.727778, y: 35.360556, z: 5000, spatialReference: AGSSpatialReference.wgs84())
        self.planeModelGraphic.geometry = startPoint
        self.planeModelGraphic.attributes["HEADING"] = 0
        self.planeModelGraphic.attributes["PITCH"] = 0
        self.planeModelGraphic.attributes["ROLL"] = 0
        
        //2D plane
        self.triangleGraphic.geometry = startPoint
        
        //set viewpoint for map view
        let viewpoint = AGSViewpoint(center: startPoint, scale: 100000)
    ...
    @objc func startMotion() {
        ...
        motionManager.startDeviceMotionUpdates(to: OperationQueue.main, withHandler: {(motionData, error) in
            ...
            print("yaw:" + String(data.attitude.yaw))
            ...
            self.planeModelGraphic.attributes["ROLL"] = roll
            
            let currentPoint = self.planeModelGraphic.geometry as! AGSPoint
            
            let newPoint = AGSPoint(x: currentPoint.x + roll/1000, y: currentPoint.y + 0.001, z: currentPoint.z + pitch * 10, spatialReference: currentPoint.spatialReference)
            
            self.planeModelGraphic.geometry = newPoint
            
            //update labels
            self.planeStatsViewController?.altitudeLabel?.text = "\(Int(newPoint.z))"
            self.planeStatsViewController?.headingLabel?.text = "\(Int(heading))º"
            self.planeStatsViewController?.pitchLabel?.text = "\(Int(pitch))º"
            self.planeStatsViewController?.rollLabel?.text = "\(Int(roll))º"
            
            self.triangleGraphic.geometry = newPoint
            let viewpoint = AGSViewpoint(center: newPoint, scale: 100000, rotation: 360 + Double(heading))
    ...
    @IBAction func changeMissionAction() {
        ...
        var missionFileName = ""
        if self.selectedMissionIndex == 0 {
            missionFileName = "GrandCanyon.csv"
        } else if self.selectedMissionIndex == 1 {
            self.playBBI.isEnabled = false
            self.operate()
            return
        } else if self.selectedMissionIndex == 2 {
            missionFileName = "Hawaii.csv"
        } else if self.selectedMissionIndex == 3 {
            missionFileName = "Pyrenees.csv"
        } else if self.selectedMissionIndex == 4 {
            missionFileName = "Snowdon.csv"
        }
